	...
	phi(adist(rng))
{
	...
	AddBind(std::make_unique<PixelShader>(d3d, L"../bin/PixelShaderps.cso"));//像素着色器
	...
	AddBind(std::make_unique<PixelConstantBuffer<ConstantBuffer2>>(d3d, cb2));

	const std::vector<D3D11_INPUT_ELEMENT_DESC> ied =
	{
		{ "Position",0,DXGI_FORMAT_R32G32B32_FLOAT,0,0,D3D11_INPUT_PER_VERTEX_DATA,0 },
	};
	AddBind(std::make_unique<InputLayout>(d3d, ied, pvsbc));//绑定layout
