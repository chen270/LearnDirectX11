...
bool D3dClass::InitComputeShaderResource()
{

	HR(DirectX::CreateDDSTextureFromFile(pDevice.Get(), L"../texture/flare.dds",
		nullptr, m_pTextureInputA.GetAddressOf()));
	HR(DirectX::CreateDDSTextureFromFile(pDevice.Get(), L"../texture/flarealpha.dds",
	...
	HR(pDevice->CreateTexture2D(&texDesc, nullptr, m_pTextureOutputA.GetAddressOf()));

	texDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	HR(pDevice->CreateTexture2D(&texDesc, nullptr, m_pTextureOutputB.GetAddressOf()));
	...
	HR(pDevice->CreateUnorderedAccessView(m_pTextureOutputA.Get(), &uavDesc,
		m_pTextureOutputA_UAV.GetAddressOf()));

	uavDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	HR(pDevice->CreateUnorderedAccessView(m_pTextureOutputB.Get(), &uavDesc,
		m_pTextureOutputB_UAV.GetAddressOf()));

	// 创建计算着色器
	ComPtr<ID3DBlob> blob;
	HR(D3DCompileFromFile(L"../shader/TextureMul_R32G32B32A32_CS.hlsl",
		nullptr,nullptr, "main", "cs_5_0",D3DCOMPILER_STRIP_DEBUG_INFO, 0, blob.GetAddressOf(), nullptr));
	HR(pDevice->CreateComputeShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pTextureMul_R32G32B32A32_CS.GetAddressOf()));

	HR(D3DCompileFromFile(L"../shader/TextureMul_R8G8B8A8_CS.hlsl",
		nullptr, nullptr, "main", "cs_5_0", D3DCOMPILER_STRIP_DEBUG_INFO, 0, blob.GetAddressOf(), nullptr));
	HR(pDevice->CreateComputeShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pTextureMul_R8G8B8A8_CS.GetAddressOf()));

	return true;
}

void D3dClass::Compute()
{
	assert(pContext);

	pContext->CSSetShaderResources(0, 1, m_pTextureInputA.GetAddressOf());
	pContext->CSSetShaderResources(1, 1, m_pTextureInputB.GetAddressOf());

	// DXGI Format: DXGI_FORMAT_R32G32B32A32_FLOAT
	// Pixel Format: A32B32G32R32
	pContext->CSSetShader(m_pTextureMul_R32G32B32A32_CS.Get(), nullptr, 0);
	pContext->CSSetUnorderedAccessViews(0, 1, m_pTextureOutputA_UAV.GetAddressOf(), nullptr);
	pContext->Dispatch(32, 32, 1);

	// DXGI Format: DXGI_FORMAT_R8G8B8A8_SNORM
	// Pixel Format: A8B8G8R8
	pContext->CSSetShader(m_pTextureMul_R8G8B8A8_CS.Get(), nullptr, 0);
	pContext->CSSetUnorderedAccessViews(0, 1, m_pTextureOutputB_UAV.GetAddressOf(), nullptr);
	pContext->Dispatch(32, 32, 1);

	HR(DirectX::SaveDDSTextureToFile(pContext.Get(), m_pTextureOutputA.Get(), L"../texture/flareoutputA.dds"));
	HR(DirectX::SaveDDSTextureToFile(pContext.Get(), m_pTextureOutputB.Get(), L"../texture/flareoutputB.dds"));
